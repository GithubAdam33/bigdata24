...
def invoke(inputs):
    ...
    save_path = '/root/mine/BDC2024/output'
    ...
    for i in range(2):
        if i == 0:
            data = np.load(os.path.join(test_data_root_path, "temp_val.npy")) # (N, L, S, 1)
        else: 
            data = np.load(os.path.join(test_data_root_path, "wind_val.npy"))
        N, L, S, _ = data.shape # 72, 168, 60
        

        cenn_era5_data = np.load(os.path.join(test_data_root_path, "global_val.npy")) 
        ...
        if i == 0:
            model.load_state_dict(torch.load("./checkpoints/v1_iTransformer_Meteorology_ftMS_sl168_ll1_pl24_dm64_nh1_el1_df64_global_temp/checkpoint_0.pth"))
        else:
            model.load_state_dict(torch.load("./checkpoints/v1_iTransformer_Meteorology_ftMS_sl168_ll1_pl24_dm64_nh1_el1_df64_global_wind/checkpoint_0.pth"))
        outputs = model(data)
...
if __name__ == '__main__':
    val_path = '/root/mine/BDC2024/dataset/north/val'
    invoke(val_path)